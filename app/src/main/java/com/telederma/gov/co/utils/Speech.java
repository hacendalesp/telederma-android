...
public class Speech {
    ...
    private Locale mLocale = new Locale("es", "ES");//Locale.getDefault();
    ...
    public void startListening(final SpeechProgressView progressView, final SpeechDelegate delegate)
        ...
        // configura los colores de la barra de escucha
        int[] colors = {
...
//        intent.putExtra(RecognizerIntent.EXTRA_CALLING_PACKAGE, this.mCallingPackage);
...
//        intent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 60000);
    ...
    public void startListening(final View rootProgressView, final SpeechDelegate delegate)
            throws SpeechRecognitionNotAvailable, GoogleVoiceTypingDisabledException {

        SpeechProgressView progressView = rootProgressView.findViewById(R.id.progress);
        if (progressView == null) {
            Log.e(this.getClass().getSimpleName(), "ProgressView no inicializado!");
            return;
        }
