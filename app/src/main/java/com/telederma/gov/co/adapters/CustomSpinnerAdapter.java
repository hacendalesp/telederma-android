...
public class CustomSpinnerAdapter<E extends BaseEntity> extends BaseAdapter implements SpinnerAdapter {

    private final Context activity;
    private List<E> data;
    private final Class<E> clazz;

    public CustomSpinnerAdapter(Context context, List<E> data, Class<E> entityClass) {
    ...
    private String obtenerTexto(E e) {
        if (e == null) {
            if (getCount() == 0) {
                return activity.getResources().getString(R.string.label_sin_controles);
            }else{
                if (getCount() > 1) {
